...
def sonProcess(ProcessID,sendPipe):
    ...
    for i in range(total):
        sleepRandom(ProcessID)
        sendPipe.send(i+1)
    sendPipe.close()

def multBar(ProcessNum,total,sendPipe,receivePipe):
    ...
    unset_win()
    print("wait Process to finish: {}".format(time2String(int(time.time()-processBeginTime))))
...
if __name__ == "__main__":
    ...
    multBar(ProcessNum,total,sendPipe,receivePipe)
